...
<script>
  ...
  $(document).ready(function() {
    ...
    table = $('#table').DataTable({
      ...
      "serverSide": true,
      "ajax": {
        "url": "<?= site_url('dashboard/transaksi/pesanan/get_datatables') ?>",
        "type": "POST"
      },
      "columnDefs": [{
        "targets": [-1],
  ...
  function save_resi() {
    ...
    $.ajax({
      url: "<?= site_url('dashboard/transaksi/pesanan/resi_update') ?>",
      type: "POST",
  ...
  function delete_order(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "<?= site_url('dashboard/transaksi/pesanan/ajax_trash') ?>",
          type: "POST",
          data: {
            id: id
          },
