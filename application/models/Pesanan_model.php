...
class Pesanan_model extends CI_Model {
  ...
  public function get_datatables() {
    ...
    foreach ($list as $ls) {
      ...
      } elseif ($ls->opsi_beli == 1) {
        $row[] = '<a href="'.site_url("dashboard/transaksi/pesanan/detail/").$ls->id_pesanan.'">'.$ls->id_pesanan.'</a>';
      } elseif ($ls->opsi_beli == 2) {
      ...
      if ($ls->status == 'tolak' || $ls->status == 'selesai' || $ls->status == 'batal') {
        ...
        $row[] = '<div class="btn-group"> <button class="btn btn-info btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Opsi</button> <div class ="dropdown-menu"><a class="dropdown-item" href="'.site_url("dashboard/transaksi/pesanan/detail/").$ls->id_pesanan.'">Tinjau</a></div> </div>';
      } elseif ($ls->status == 'terima') {
        $row[] = '<div class="btn-group"> <button class="btn btn-info btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Opsi</button> <div class ="dropdown-menu"><a class="dropdown-item" href="javascript:void(0)" onclick="edit('."'".$ls->id_pesanan."'".','."'".$ls->status."'".','."'".$ls->resi."'".')">Kirim Sekarang</a></div> </div>';
      } elseif ($ls->status == 'kirim') {
        $row[] = '<div class="btn-group"> <button class="btn btn-info btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Opsi</button> <div class ="dropdown-menu"><a class="dropdown-item" href="'.site_url("dashboard/transaksi/pesanan/tracking/").$ls->id_pesanan.'">Lacak Pesanan</a> <a class="dropdown-item" onclick="edit_resi('."'".$ls->id_pesanan."'".','."'".$ls->resi."'".')">Edit Resi</a> <a class="dropdown-item" href="javascript:void(0)" onclick="selesai('."'".$ls->id_pesanan."'".','."'".$ls->status."'".','."'".$ls->resi."'".')">Selesai</a></div> </div>';
      }
      $data[] = $row;
    }
  ...
  */
  ...
  public function report($tanggalawal = null, $tanggalakhir = null, $opsi_beli = '') {
    if ($opsi_beli != '') {
      $this->db->where('opsi_beli', $opsi_beli);
    }
    if ($tanggalawal && $tanggalakhir) {
      $this->db->where('tgl_bayar_pesanan >=', "$tanggalawal 00:00:00");
      $this->db->where('tgl_bayar_pesanan <=', "$tanggalakhir 23:59:59");
    }
    $this->db->order_by('tgl_bayar_pesanan', 'asc');
    $this->db->from($this->table);
