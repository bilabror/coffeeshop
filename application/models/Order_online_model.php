...
class Order_online_model extends CI_Model {
  ...
  public function get_datatables() {
    ...
    foreach ($list as $ls) {
      ...
      $row[] = '<a href="'.site_url("dashboard/transaksi/online/detail/").$ls->id_pesanan.'">'.$ls->id_pesanan.'</a>';
      ...
      if (date("Y-m-d", strtotime($ls->tgl_bayar_pesanan)) == date('Y-m-d')) {
      ...
      if ($ls->status == 'tolak' || $ls->status == 'selesai' || $ls->status == 'batal') {
        ...
        $row[] = '<div class="btn-group"> <button class="btn btn-info btn-sm dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Opsi</button> <div class ="dropdown-menu"><a class="dropdown-item" href="'.site_url("dashboard/transaksi/online/detail/").$ls->id_pesanan.'">Tinjau</a></div> </div>';
